...
/**
 * Демонстрационный класс для тестирования базовых структур данных и фильтров
 * RBAC
 */
public class Main {
  // Форматер для создания метаданных с кастомной датой (только для
  // демонстрации)
  private static final DateTimeFormatter FORMATTER =
      DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");

  // Вспомогательный метод для создания метаданных с заданной датой (для тестов
  // фильтров по дате)
  private static AssignmentMetadata createMetadata(String assignedBy,
  ...
  public static void main(String[] args) {
    System.out.println(
        "╔════════════════════════════════════════════════════════════════╗");
    System.out.println(
        "║        ДЕМОНСТРАЦИЯ RBAC: БАЗОВЫЕ СТРУКТУРЫ И ФИЛЬТРЫ         ║");
    System.out.println(
        "╚════════════════════════════════════════════════════════════════╝\n");
  ...
  private static void testBaseStructures() {
    ...
    Permission p1 =
        new Permission("read", "USERS", "Просмотр списка пользователей");
    Permission p2 =
        new Permission("WRITE", "reports", "Редактирование отчётов");
    ...
    AssignmentMetadata meta =
        AssignmentMetadata.now("администратор", "Первоначальная настройка");
    System.out.println(meta.format());

    System.out.println("\n=== ТЕСТ 1.7: Постоянное назначение ===");
    User admin =
        User.validate("admin", "Администратор Системы", "admin@company.com");
    ...
    AssignmentMetadata tempMeta =
        AssignmentMetadata.now("менеджер", "Временный доступ для аудита");
    TemporaryAssignment ta =
        new TemporaryAssignment(admin, viewer, tempMeta, "2026-12-31 23:59");
  ...
  private static void testUserFilters() {
    ...
    UserFilter combined = UserFilters.byFullNameContains("ан").and(
        UserFilters.byEmailDomain("@company.com"));
    users.stream().filter(combined).forEach(
        u -> System.out.println("  ✓ " + u.fullName()));
  }
  ...
  private static void testRoleFilters() {
    ...
    editorRole.addPermission(
        new Permission("WRITE", "articles", "Редактирование статей"));
    editorRole.addPermission(
        new Permission("PUBLISH", "articles", "Публикация статей"));

    Role viewerRole = new Role("Наблюдатель", "Только просмотр");
    viewerRole.addPermission(
        new Permission("READ", "reports", "Просмотр отчётов"));
  ...
  private static void testAssignmentFilters() {
    ...
    AssignmentMetadata meta1 =
        createMetadata("admin", "Тест", "2026-01-15 10:00");
    AssignmentMetadata meta2 =
        createMetadata("manager", "Тест", "2026-02-20 14:30");
    AssignmentMetadata meta3 =
        createMetadata("admin", "Тест", "2026-03-10 09:15");
    ...
    TemporaryAssignment ta2 = new TemporaryAssignment(
        user2, role1, AssignmentMetadata.now("admin", "Будущее назначение"),
        "2027-12-31 23:59");
    ...
    assignments.forEach(a
                        -> System.out.println("  • [" + a.assignmentType() +
                                              "] " + a.user().username() +
                                              " → " + a.role().name()));
    ...
    assignments.stream()
        ...
        .forEach(a
                 -> System.out.println(
                     "  ✓ " + a.summary().split("\n")[0] +
                     " | Истекает: " + ((TemporaryAssignment)a).expiresAt()));
  ...
  private static void testSorting() {
    ...
    List<User> users =
        Arrays.asList(User.validate("zebra", "Зебра", "z@company.com"),
    ...
    List<RoleAssignment> assignments =
        Arrays.asList(new PermanentAssignment(dummyUser, dummyRole, m1),
